  ...
  notifModels: (id) => {
    return new Promise((resolve, reject) => {
      connection.query(
        ...
        `SELECT image_recruiter, company_name, jobs_needed, hired_jobs.created_at FROM user_account JOIN hired_jobs ON id_user = id_recruiter JOIN profile_recruiter ON hired_jobs.id_recruiter = profile_recruiter.id_recruiter WHERE id_pekerja = ${id} && roles = 1`,
